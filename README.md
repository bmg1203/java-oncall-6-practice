...
### 입력 기능
* 월, 요일 입력
  * 쉼표(,)를 기준으로 월과 요일을 입력한다. 
  * 월은 숫자로 입력한다. 
  * 요일은 요일 없이 입력한다. 
* 평일 & 주말 근무 순서 입력
  * 평일과 주말 근무 순서를 쉼표(,)를 기준으로 입력한다. 

### 입력 유효성 검사 기능
> 입력이 유효하지 않은 경우 `IllegalArgumentException`을 발생시키고
> `[ERROR] 유효하지 않은 입력 값입니다. 다시 입력해 주세요.`를 출력한다. 
* 월
  * 1~12 사이의 숫자를 입력한다.
* 요일
  * 월, 화, 수, 목, 금, 토, 일 중에 하나로 입력한다. 
* 평일 & 주말 근무
  * 각 근무에 중복된 이름은 불가능 하다. 
  * 각 근무에는 최소 5명 최대 35명이 배치된다. 
  * 5자 이하의 이름이 유효하다. 
  * 유효하지 않다면 평일부터 다시 입력한다. 

### 월별 일수 계산 기능
* 1~12월 월당 해당하는 마지막 날짜를 계산한다. 
* 2월은 28일로 계산한다. 

### 요일에 대한 순서 판단 기능
* 요일에 해당하는 숫자를 이용한다. 0~6(월~일)
* 입력받은 요일을 기준으로 요일을 추가해나간다. 
* 일요일이 지나면 다시 월요일로 돌아간다. 
...
### 중복 근무 판단 기능
* 근무는 2일 연달아 불가능하다. 
* 이 경우 다음 근무자와 근무를 교체하여 근무한다. 

### 출력 기능
* 달, 일, 요일, 휴일 유무, 근무자 순으로 출력한다. 
* 법정 공휴일인 경우 평일이면 `(휴일)`을 붙여 출력한다. 